...
def play_game(env, model):
    model.load_weights("torcs_weights.h5")
...
# #create model
# S = Input(shape=(29, ))   
# h0 = Dense(50, activation='relu', kernel_initializer='glorot_normal')(S)
# h1 = Dense(100, activation='relu', kernel_initializer='glorot_normal')(h0)
...
init_pop = []
for i in range(10):
    model.load_weights("{cc}.h5".format(cc=i))
    #pygad.kerasga.model_weights_as_vector(model=model)
    init_pop.append(pygad.kerasga.model_weights_as_vector(model=model))
